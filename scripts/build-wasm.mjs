#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-dart.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-dart-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/UserNobody14/tree-sitter-dart.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as dart.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "dart.wasm"));
const bytes = (await readFile(path.join(repoRoot, "dart.wasm"))).length;
console.log(`dart.wasm: ${bytes} bytes (built from ${pin})`);
