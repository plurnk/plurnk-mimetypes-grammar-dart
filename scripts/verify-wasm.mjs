#!/usr/bin/env node
// CI byte-identical reproducibility check for dart.wasm.
...
const committed = await readFile(path.join(repoRoot, "dart.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed dart.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-dart-verify-"));
...
console.log(`rebuilt dart.wasm sha256: ${rebuiltHash}`);
